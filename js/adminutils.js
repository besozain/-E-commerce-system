...
const displayPaginationProduct = function (ourData, currentPage, itemsPerPage, targetTable, name, price, category , inStock) {
...
//search
const searchInputFunc = function (searchInput, targetTable, ourData, name, price, category , inStock) {
    let match = false;
...
const products = function(fetchURL, keylocal, name, price, category , inStock) {
    ...
    this.fetch(fetchURL).then((Response) => {
        Response.json().then((data) => {
            ...
            console.log(ourData)
            displayPaginationProduct(ourData, currentPage,itemsPerPage,targetTable, name, price, category , inStock )
            ...
            btns[Math.ceil(ourData.length / itemsPerPage)+1].addEventListener('click' ,function() {
                if(currentPage == Math.ceil(ourData.length / itemsPerPage)) {return;}
                currentPage +=1;  
                resetBtnColor(btns)        
                currentBtnColor(btns[currentPage])          
                displayPaginationProduct(ourData, currentPage,itemsPerPage,targetTable);
    ...
    searchBtn.addEventListener("click" , function() {
        ...
        searchInputFunc(searchInput.value,targetTable, ourData, name, price, category , inStock)
